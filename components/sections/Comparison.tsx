      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-brand-orange/10 p-10 rounded-[2.5rem] border border-brand-orange/20">
            <span className="text-brand-orange/80 font-bold uppercase tracking-widest text-xs mb-6 block">{siteConfig.name}</span>
            <ul className="space-y-4">
              {siteConfig.comparison.flowingMonkey.map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-white/80">
                  <Check className="text-brand-orange/80 w-5 h-5 flex-shrink-0" /> {item}
                </li>
              ))}
            </ul>
          </div>
